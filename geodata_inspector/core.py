...
Usage:
    python inspect_geodata_duckdb.py [file_path]

To benchmark against the original:
    python inspect_geodata_duckdb.py --benchmark [file_path]
"""
...
def _compute_spatial_metrics_duckdb(conn, table_name, geom_type, gdf_metro, start_time):
    ...
    # Compute area based on geometry type
    if 'POINT' in geom_type.upper():
        # For points: use convex hull
        hull_result = conn.execute(f"""
        ...
        """).fetchone()
        area_km2 = hull_result[0] if hull_result[0] else 0
        # Compute average complexity (number of points)
        ...
        try:
            area_result = conn.execute(f"""
                ...
                FROM {table_name}
            """).fetchone()
            area_km2 = area_result[0] if area_result[0] else 0
        except Exception:
            # Fallback to convex hull if union fails
            hull_result = conn.execute(f"""
                SELECT ST_Area(ST_ConvexHull(ST_Union_Agg(geom))) / 1e6
                FROM {table_name}
            """).fetchone()
            area_km2 = hull_result[0] if hull_result[0] else 0
    ...
    # Compute coverage against reference
    coverage_pct = 0
    try:
        hull_wkt = conn.execute(f"""
            SELECT ST_AsText(ST_ConvexHull(ST_Union_Agg(geom))) FROM {table_name}
        """).fetchone()[0]
    ...
    return {
        ...
        "Complexite moyenne": complexity,
        "Geometries dupliquees (%)": round(dup_pct, 2),
...
def inspect_csv_duckdb(filepath, gdf_metro):
    ...
    summary_rows.append({**base_summary, **geo_summary})
    print(f"\n{filepath} done\n")
...
def inspect_excel(filepath, gdf_metro, sample_size=5000):
    ...
    import inspect_geodata_optimized as original
    import openpyxl

    try:
        # Step 1: Quick scan to find best sheet and header row (from original)
        wb = openpyxl.load_workbook(filepath, data_only=True, read_only=True)
        sheet_names = wb.sheetnames
        best_sheet = original.find_best_data_sheet(wb, sheet_names)
        ws = wb[best_sheet]

        total_rows = ws.max_row or 0
        total_cols = ws.max_column or 0
        header_row = original.find_header_row(ws, max_rows_to_check=30)
        wb.close()
        ...
        if total_data_rows <= sample_size:
            ...
            res_geom = original.get_geo_columns(df_sample)
        ...
        df, _ = original.fix_insee_codes(df)

        # Get geo columns if not already detected
        if res_geom is None:
            res_geom = original.get_geo_columns(df)

        geo_keys = original.detect_geo_join_keys(df)
        ...
        if res_geom['columns'] and res_geom['method'] == 'points_from_xy':
            ...
            geo_summary['Geometries dupliquees (%)'] = round(duplicate_pct, 2)  # Use pre-computed value
            ...
            geo_summary['Geometries dupliquees (%)'] = round(duplicate_pct, 2)
            base_summary["Type de fichier"] = f"EXCEL with Geometry (DuckDB Spatial) ({sheet_info})"
            ...
            print(f"[DuckDB] Geometry: {res_geom['columns']} ({res_geom['method']})")
            gdf = original.create_geodataframe_from_result(df, res_geom)
            gdf_proj, geo_metrics = process_geodataframe(gdf, gdf_metro, compute_duplicates=False)

            if geo_metrics:
                geo_metrics['Geometries dupliquees (%)'] = round(duplicate_pct, 2)
                geo_summary = geo_metrics
        ...
        print(f"[DuckDB] Excel processing error: {e}")
        import traceback
        traceback.print_exc()
        # Fall back to original implementation
        print("[DuckDB] Falling back to original implementation...")
        original.summary_rows.clear()
        original.last_gdf = None
        original.inspect_excel(filepath, gdf_metro, sample_size)
        if original.summary_rows:
            summary_rows.append(original.summary_rows[-1])
        if original.last_gdf is not None:
            last_gdf = original.last_gdf
...
def inspect_geospatial_duckdb(filepath, gdf_metro):
    ...
    try:
        ...
        geom_col = None
        for _, row in schema.iterrows():
            if 'GEOMETRY' in row['column_type'].upper():
                geom_col = row['column_name']
                break

        if geom_col is None:
            geom_col = 'geom'  # Default DuckDB spatial column name
...
def process_geodataframe(gdf, gdf_metro, compute_duplicates=True):
    ...
    try:
        ...
        doublons = pourcentage_geometries_dupliquees(gdf_proj) if compute_duplicates else {'Geometries dupliquees (%)': 0}
        ...
        geo_metrics = {
            ...
            "Emprise estimée (km2)": round(area_km2, 2),
            "Densité (obj/km2)": round(density, 2),
            "Taux de remplissage (%)": remplissage['Taux de remplissage (%)'],
            "Complexite moyenne": complexite['Complexite moyenne (sommets)'],
            "Geometries dupliquees (%)": doublons['Geometries dupliquees (%)'],
...
def get_default_geo_summary():
    ...
    return {
        ...
        "Taux de remplissage (%)": None,
        "Complexite moyenne": None,
        "Geometries dupliquees (%)": None,
...
def inspect_file(filepath, gdf_metro):
    ...
    if ext in ['.csv', '.txt']:
        ...
        inspect_excel_duckdb(filepath, gdf_metro)


def benchmark_comparison(filepath, gdf_metro):
    """
    Run both original and DuckDB versions and compare performance.
    """
    import inspect_geodata_optimized as original

    print("=" * 60)
    print("BENCHMARK: Original vs DuckDB")
    print("=" * 60)
    ...
    # Test original
    print("--- ORIGINAL (pandas/geopandas) ---")
    original.summary_rows.clear()
    original.last_gdf = None

    start = time.time()
    original.inspect_file(filepath, gdf_metro)
    ...
    start = time.time()
    inspect_file(filepath, gdf_metro)
    ...
    if duckdb_time < original_time:
        speedup = original_time / duckdb_time
        print(f"DuckDB is {speedup:.1f}x FASTER")
    else:
        slowdown = duckdb_time / original_time
        print(f"DuckDB is {slowdown:.1f}x slower")

    return {
        'original_time': original_time,
        'duckdb_time': duckdb_time,
        'speedup': original_time / duckdb_time if duckdb_time > 0 else 0
    }
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python inspect_geodata_duckdb.py <file_path>           # Inspect file")
        print("  python inspect_geodata_duckdb.py --benchmark <file>    # Compare performance")
        print()
        print("Examples:")
        print("  python inspect_geodata_duckdb.py data/pesticides-2002-2022-v03-2024-vf.csv")
        print("  python inspect_geodata_duckdb.py --benchmark data/pesticides-2002-2022-v03-2024-vf.csv")
        sys.exit(1)

    if sys.argv[1] == "--benchmark":
        if len(sys.argv) < 3:
            print("Error: Please provide a file path for benchmarking")
            sys.exit(1)
        benchmark_comparison(sys.argv[2], gdf_reference)
    else:
